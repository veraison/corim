...
func validateCCASignerID(keys *comid.CryptoKeys, tripleIndex, measIndex int) error {
	...
	// a) CryptoKeys field MUST be set (mandatory per CCA Platform profile)
	if keys == nil {
	...
	// b) The CryptoKey must be of type "bytes" (TaggedBytes, CBOR tag 560)
	if key.Type() != "bytes" {
		return fmt.Errorf("%s: cryptokeys (signer-id) must be of type 'bytes', got '%s'", prefix, key.Type())
	}

	// c) Byte length must be 32, 48, or 64 (SHA-256, SHA-384, or SHA-512)
	...
	signerIDLen := len(tb)
	if signerIDLen != 32 && signerIDLen != 48 && signerIDLen != 64 {
		return fmt.Errorf("%s: signer-id must be 32, 48, or 64 bytes, got %d", prefix, signerIDLen)
	}
...
func validateCCAPlatformImplementationID(env *comid.Environment, prefix string) error {
	...
	if env.Class == nil {
		return fmt.Errorf("%s: environment.class is required for CCA Platform profile", prefix)
	}

	if env.Class.ClassID == nil {
		return fmt.Errorf("%s: environment.class.id (implementation-id) is required for CCA Platform profile", prefix)
	}
	...
	if len(idBytes) != PlatformImplIDSize {
		return fmt.Errorf("%s: implementation-id must be exactly %d bytes, got %d", prefix, PlatformImplIDSize, len(idBytes))
	}
...
func validateCCAPlatformInstanceID(env *comid.Environment, prefix string) error {
	...
	if env.Instance == nil {
		return fmt.Errorf("%s: environment.instance (instance-id) is required for CCA Platform profile", prefix)
	}
	...
	// CCA Platform Instance ID must be exactly 33 bytes (1 byte RAND type + 32 bytes identifier)
	if len(ueidBytes) != PlatformInstanceIDSize {
		return fmt.Errorf("%s: instance-id must be exactly %d bytes, got %d", prefix, PlatformInstanceIDSize, len(ueidBytes))
	}
